...
def walk_forward_allocations(features, targets, strategy_names, train_min_days=365, prediction_frequency="W-FRI", model_name="logistic_regression", label_horizon_days=7):
    ...
    for date in pred_dates:
        # A target stamped t contains returns through t + label_horizon_days.
        # Purge labels whose outcome window is not fully known before prediction.
        train_idx = X.index[X.index < date - pd.Timedelta(days=label_horizon_days)]

...
def run(config_path="configs/default.yaml", data_path=None, top_n=20, model_name="logistic_regression", cost_bps=25, use_reasoning=True):
    ...
    sret = strategy_return_frame(panel, universes); feats = build_features(panel, sret, universes); target = make_strategy_targets(sret)
    ...
    benches = benchmark_returns(panel, sret, universes)

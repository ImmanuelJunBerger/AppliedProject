"""CLI entrypoint for HMM/GMM macro-regime classification study."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run HMM/GMM/KMeans macro-regime classification overlays for BTC/ETH/cash allocation."
    )
    ...
    parser.add_argument(
        "--output-dir",
        default="reports/macro_regime_hmm_gmm",
...
def main() -> None:
    ...
    write_macro_regime_hmm_gmm_reports(args.output_dir, result)
    recommendation = result["recommendation"]
    print(f"HMM/GMM macro-regime reports written to {Path(args.output_dir)}")
    print(f"Development-selected candidate: {recommendation['development_selected_candidate']}")

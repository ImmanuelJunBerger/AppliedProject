"""CLI entrypoint for macro-regime strategy v3 simplification study."""
from __future__ import annotations

import argparse
from pathlib import Path

from crypto_mlsystem.macro_regime_benchmark_analysis import load_default_inputs
from crypto_mlsystem.macro_regime_strategy_v3 import (
    run_macro_regime_strategy_v3,
    write_macro_regime_strategy_v3_reports,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run constrained macro-regime simplification study for btc_eth_macro_gate_balanced."
    )
    parser.add_argument("--data", default="data/binance_usdt_broad_2019.csv", help="Crypto OHLCV panel CSV.")
    parser.add_argument(
        "--macro-features",
        default="data/processed/wrds_macro_features/wrds_macro_features_daily.csv",
        help="Daily WRDS macro feature CSV.",
    )
    parser.add_argument(
        "--public-data-dir",
        default="data/processed/public_crypto",
        help="Processed public crypto-native data directory.",
    )
    parser.add_argument(
        "--volatility-predictions",
        default="reports/volatility_expansion/predictions.csv",
        help="Existing volatility-expansion prediction CSV.",
    )
    parser.add_argument(
        "--output-dir",
        default="reports/macro_regime_strategy_v3",
        help="Output report directory.",
    )
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    panel, macro, public_data, probability = load_default_inputs(
        data_path=args.data,
        macro_path=args.macro_features,
        public_data_dir=args.public_data_dir,
        volatility_predictions=args.volatility_predictions,
    )
    result = run_macro_regime_strategy_v3(panel, macro, public_data, probability)
    write_macro_regime_strategy_v3_reports(args.output_dir, result)
    winner = result["winner"]
    recommendation = result["recommendation"]
    stats = result["statistics"]
    selected_stats = stats[stats.candidate == winner.name].iloc[0]
    print(f"Macro-regime v3 reports written to {Path(args.output_dir)}")
    print(f"Development-selected candidate: {winner.name}")
    print(f"Decision: {recommendation['decision']}")
    print(f"Restricted-family PBO: {selected_stats.family_pbo:.2%}")


if __name__ == "__main__":
    main()

...
def test_write_ohlcv_csv_uses_utf8(tmp_path):
    path = write_ohlcv_csv([{"date": "2020-01-01", "symbol": "非ASCII", "open": 1, "high": 2, "low": 0.5, "close": 1.5, "volume": 10, "market_cap": 15}], tmp_path / "unicode.csv")
    assert "非ASCII" in Path(path).read_text(encoding="utf-8")
